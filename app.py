...
# ---------------------------------------------------------------
# VULNERABILITY 1 (NEW - SonarQube Detection): Command Injection
# This endpoint accepts a 'host' parameter and passes it directly
# to os.system() without any sanitisation - a clear OS command
# injection vulnerability (CWE-78). SonarQube flags this as a
# Critical Security Hotspot.
# ---------------------------------------------------------------
@app.route('/api/ping', methods=['GET'])
def ping_host():
    host = request.args.get('host', '')
    # VULNERABILITY: Direct use of user input in system command
    result = os.system(f"ping -c 1 {host}")
    return jsonify({"result": result})


# ---------------------------------------------------------------
# VULNERABILITY 2 (ZAP Detection): Reflected XSS
# This endpoint reflects the 'q' query parameter directly into
# an HTML response without encoding. ZAP's active scanner will
# inject payloads like <script>alert(1)</script> and detect the
# unescaped reflection (CWE-79).
# ---------------------------------------------------------------
@app.route('/api/search', methods=['GET'])
def search():
    query = request.args.get('q', '')
    # VULNERABILITY: Unsanitised user input reflected in HTML response
    # No Content-Security-Policy or X-XSS-Protection headers either
    html_response = f"""
    <html>
      <body>
